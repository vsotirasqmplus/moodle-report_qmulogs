...
$strings = get_string_manager()->load_component_strings('report_qmulogs', current_language());
/**
 * @return string
 */
function get_preference_name(): string
{
 ...
 * @return string
 */
function get_logs_sql($form_data): string
{
	global $strings;
	$table_name = get_standard_log_table_name();
	...
	$table_fields = implode(', ', array_keys(get_table_fields()));
 ...
 * @return string
 */
function get_default_preferences(): string
{
	$fields = [];
	foreach(get_table_fields() as $name => $field){
		$fields['field_' . $name] = 1;
	}
	return encode_preferences($fields);
}
 ...
 * @return array
 */
function get_table_fields($table_name = NULL): array
{
	global $DB;
	$fields = [];

	try {
		$table_name = $table_name ?? get_standard_log_table_name();
 ...
 * @return bool
 */
function form_store_data(string $pref_name, $data): bool
{
	global $USER;
	try {
		set_user_preference($pref_name, encode_preferences($data), (int)$USER->id);
	} catch(Exception $exception) {
		return FALSE;
	}
	return TRUE;
 ...
 * @return string[]
 */
function mandatory_fields(): array
{
 ...
 * @return array
 */
function get_logstore_names(): array
{
	# global $CFG;
	$table_names = [];
 ...
 */
function get_table_foreign_keys_mysql($database_name, $table_name): array
{
	global $strings;
	$relations = [];
 ...
 * @return string
 *
 * The zipping and encoding are utilised to minimise and standardise the set of preferences in large objects or arrays
 *               as the preference field is of limited length
 */
function encode_preferences($data)
{
 ...
 * @return mixed
 */
function decode_preferences($data)
{
 ...
 * @return array
 */
function get_teacher_non_student_core_capabilities(): array
{
	global $DB;
	$return = [];
 ...
 * @return string
 */
function get_standard_log_table_name(): string
{
	global $CFG;
	$table_name = '';
	try {
		...
		if($lines) {
			...
			if($table_name > '') {
				$table_name = table_exists($table_name) ? $table_name : '';
			}
		}
	} catch(Exception $exception) {
		error_log($exception->getMessage() . $exception->getTraceAsString());
 ...
 * @return array
 */
function get_course_logs(string $sql)
{
	global $DB;
	try {
		return $DB->get_records_sql($sql);
 ...
 * @return array
 */
function get_course_teacher_ids(int $courseid): array
{
	global $DB;
	$teachers = [];
 ...
 * @return bool
 */
function is_teacher_in_list(object $user, array $teacher_records): bool
{
	$response = FALSE;
